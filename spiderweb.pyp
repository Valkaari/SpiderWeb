...
class SPIDERWEB_GENERATOR(plugins.ObjectData):
    ...
    def GetVirtualObjects(self, op, hierarchyhelp):
        ...
        if op[c4d.DRAWPLANE]:
            ...
            plane[c4d.PRIM_PLANE_HEIGHT] = 1000
            plane.InsertUnder(container)


                 
        swc = ComputeSpiderWeb( hub    = op, 
                                subdiv = op[c4d.SUBDIVISION],
                                radius = op[c4d.RADIUS_WEB],
                                seed   = op[c4d.SEED_ID],
                                desiredAnchorsCnt = op[c4d.ANCHOR_POINTS_CNT],
                                radii = op[c4d.RADII_CNT],
                                rotCnt = op[c4d.OS_REVOLUTION_CNT],
                                amax=op[c4d.OS_SPACE],
                                bmax=op[c4d.OS_TURBULENCE],
                                startDistance=op[c4d.START_DISTANCE],
                                offsetExtSpiral = op[c4d.OS_OFFSET_START],
                                lengthWeight = op[c4d.OS_LENGTH_WEIGHT],
                                rndHubWeight = op[c4d.RND_CENTER_DISTANCE],
                                createCS = op[c4d.CREATE_CENTRAL_SPIRAL],
                                csRotCnt = op[c4d.CS_REVOLUTION_CNT],
                                csOffSet = op[c4d.CS_OFFSET_START],
                                csamax = op[c4d.CS_SPACE],
                                csbmax = op[c4d.CS_TURBULENCE],
                                cslgthWei  = op[c4d.CS_LENGTH_WEIGHT]
